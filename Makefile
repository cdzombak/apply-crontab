...
build: ## Build for all supported platforms & architectures to ./out
	mkdir -p out
	cp ./apply-crontab.sh ./out/${BIN_NAME}-${BIN_VERSION}-all
	sed -i 's/__APPLY_CRONTAB_VERSION__="<dev>"/__APPLY_CRONTAB_VERSION__="${BIN_VERSION}"/g' ./out/${BIN_NAME}-${BIN_VERSION}-all
